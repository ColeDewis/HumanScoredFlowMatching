...
# TODO: refactor to a preprocess_point_cloud module.
...
# IDP3 realsense module does the following:
# 1. clip based on distance -> 2. grid sample (voxel sample) -> 3. uniform sample to the desired size (or pad to add extras).
# One can also note that the model training code has similar logic if you don't do this in the data loading/preprocessing step.
...
# # Calculate 3D coordinates# grid sample (voxel sample) -> uniform sample to the desired size.
...
def preprocess_point_cloud(points, use_cuda=True):
    ...
    WORK_SPACE = [[-0.5, 0.35], [-1.5, 0], [-0.6, 1]]
...
if __name__ == "__main__":
    ...
    demo_dirs = [
        os.path.join(expert_data_path, str(d))
        for d in dirs
        if os.path.isdir(os.path.join(expert_data_path, str(d)))
    ]
    ...
    for i, demo_dir in enumerate(demo_dirs):
        ...
        demo_timesteps = sorted(
            [
                int(d)
                for d in os.listdir(demo_dir)
                if os.path.isdir(os.path.join(demo_dir, str(d)))
            ]
        )
        for step_idx in tqdm.tqdm(range(len(demo_timesteps))):
            ...
            state_info = np.load(
                os.path.join(timestep_dir, "low_dim.npy"), allow_pickle=True
            ).item()
            gripper_action = state_info["joints"]["gripper_action"]
            action = list(
                state_info["cartesian"]["position"]
            )  # TODO: change to abs position
    ...
    if HAS_WEIGHTS:
        cprint(
            f"weights shape: {weight_arrays.shape}, range: [{np.min(weight_arrays)}, {np.max(weight_arrays)}]",
            "green",
        )
